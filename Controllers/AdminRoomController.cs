        ...
        public async Task<IActionResult> Create(Room room, List<int> selectedAmenities, List<IFormFile> ImageFile, List<string> selectedAmenitiesThemes, List<int> PropertyIds, bool Status = false)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    // Xử lý thuộc tính phòng (RoomProperty)
                    if (PropertyIds != null && PropertyIds.Count > 0)
                    {

                        if (room.RoomWithRoomProperties == null)
                        {
                            room.RoomWithRoomProperties = new List<RoomWithRoomProperty>();
                        }

                        foreach (var propertyId in PropertyIds)
                        {
                            var roomProperty = new RoomWithRoomProperty
                            {
                                RoomId = room.Id,
                                RoomPropertyId = propertyId
                            };
                            room.RoomWithRoomProperties.Add(roomProperty);
                            _context.RoomWithRoomProperties.Add(roomProperty);
                        }
                        await _context.SaveChangesAsync();
                    }
